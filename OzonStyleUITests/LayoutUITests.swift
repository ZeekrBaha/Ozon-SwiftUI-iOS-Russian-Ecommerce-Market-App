...
// Layout / geometry tests — automate the visual "red-lines" that were previously
// checked by hand against the reference screenshots, using XCUIElement.frame
// instead of pixel snapshots (keeps the project dependency-free).
final class LayoutUITests: XCTestCase {
    ...
    // Tab bar pinned to the bottom edge.
    ...
    // Red-line #1 — brand pill sits below the safe area, horizontally centered.
    ...
    // Red-line #2 — Favorites featured card is compact (< 60% width) and left-aligned.
    ...
    // Home "Рекомендуем" grid is 2 columns (top-row cards share a y, each < 55% wide).
    ...
    // Red-line #5 — Catalog grid is 3 columns (top-row cards share a y, each < 38% wide).
