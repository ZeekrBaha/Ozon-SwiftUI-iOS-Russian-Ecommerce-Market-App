...
// Data-access seam (MVVM-C "Model" boundary). ViewModels depend on this
// protocol, never on `SampleData` directly — so the mock source can be swapped
// for a network/database layer without touching any ViewModel or View.
...
// `SampleData` remains the single source of truth (red-line #7); it is just no
// longer referenced from the view layer.
